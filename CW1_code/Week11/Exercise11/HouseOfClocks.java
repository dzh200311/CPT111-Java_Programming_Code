...
public class HouseOfClocks {
    ...
    public void printClocksCollection() {
        for (int i = 0; i < clocksCollection.size(); i++) {
            System.out.println("Clock " + clocksCollection.get(i));
        }
    }
    ...
    public int numEarlierClocks(Clock that) {
		int numEarlierClocks = 0;
        for (int i = 0; i < clocksCollection.size(); i++) {
            if (clocksCollection.get(i).isEarlierThan(that)) {
                numEarlierClocks++;
            }
        }
		return  numEarlierClocks;
    }
